<div class="space-y-6">
    ...
    <div>
        <h2 class="text-xl font-semibold">{{ __('gatekeeper::gatekeeper.approval.Pending Approvals') }}</h2>
                ...
                <li class="p-4 bg-yellow-100 border-l-4 border-yellow-500 rounded">
                    <div class="flex items-center justify-between">
                        <div>
                            ...
                            <p class="text-xs text-gray-600">{{ __('gatekeeper::gatekeeper.approval.Requested by:') }} {{ $approval->requested_by }}</p>
                        </div>
                        <span class="text-yellow-600 text-sm">{{ __('gatekeeper::gatekeeper.approval.Pending') }}</span>
                    </div>
                </li>
            @endforeach
        </ul>
    ...
    <!-- Riwayat Persetujuan -->
    <div>
        <h2 class="text-xl font-semibold">{{ __('gatekeeper::gatekeeper.approval.Approval History') }}</h2>
            ...
            @foreach($approvalHistory as $history)
                <div class="relative">
                    ...
                    <div class="ml-6 p-4 bg-gray-100 rounded shadow">
                        ...
                        <p class="text-xs text-gray-600">{{ __('gatekeeper::gatekeeper.approval.Reviewed by:') }} {{ $history->reviewed_by }} - <span class="font-bold">{{ __(ucfirst($history->status)) }}</span></p>
