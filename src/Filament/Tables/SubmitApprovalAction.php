        ...
        try {
            if (method_exists($record, 'submitForApproval')) {
                ...
                Notification::make()
                    ...
                    ->success()
                    ->body(__('gatekeeper::gatekeeper.submit_approval_action.success_message'))
                    ->send();
            } else {
                Notification::make()
                    ->title(__('gatekeeper::gatekeeper.submit_approval_action.error_title'))
                    ->danger()
                    ->body(__('gatekeeper::gatekeeper.submit_approval_action.error_message'))
                    ->send();
            }
        } catch (\Exception $e) {
            Notification::make()
